...
contract BaseLiquidator {
	...
	function _buyrisky(address _vault, uint256 _vaultID) internal {
		...
		vault.paybackTokenAll(newVaultID, block.timestamp+10000, 0);
		vault.withdrawCollateral(newVaultID, vault.vaultCollateral(newVaultID));
	}
	...
	/**
	 * @notice Calculates the gain ratio of a vault
	 * @param _vault address of vault contract
	 * @return The gain ratio of the vault
	 */
